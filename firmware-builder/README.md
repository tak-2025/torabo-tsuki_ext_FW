...
- **コンボは keymap に `combos{}` を書かないこと。** ダイナミックコンボ（`torabo-combos`）を使う構成では、
  純正の `combos{}` ブロックと二重所有になりビルド不能・競合の原因になります。値は焼いた後 BLE / USB で編集します。
- **デバイス名を変えたら再ペアリングが必要です。** ホストは旧名をキャッシュするため、各ホストで既存のペアリングを
  削除してから再ペアリングしてください（再ビルド＋再フラッシュも必要）。16文字を超える名前は広告パケットに収まらず、
  一部ホストで表示が切れることがあります（27文字超は `CONFIG_BT_DEVICE_NAME_MAX` も自動出力）。

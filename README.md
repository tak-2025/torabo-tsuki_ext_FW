...
> 手で選ぶ代わりに **[firmware-builder](https://tak-2025.github.io/torabo-tsuki_ext_FW/)（下記）** が構成に応じた正しいスニペットの組み合わせを生成します。
...
まず本体を自分の GitHub に fork します:
[sekigon-gonnoc/zmk-keyboard-torabo-tsuki-lp](https://github.com/sekigon-gonnoc/zmk-keyboard-torabo-tsuki-lp)
→ 例 `you/zmk-keyboard-torabo-tsuki-lp`
...
構成に応じた overlay/CONFIG の組み合わせ（トポロジー）を自動で解決するので、手順1〜2 をまとめて自動化できます。
...
> private リポの場合、GitHub Actions ビルドには west clone 用のトークン設定が必要です
> （ローカル `west update` なら gh 認証で通ります）。public 化すれば不要。
...
上記を push すれば GitHub Actions が、ローカルなら `west build` がファームを生成します。
できた central 用 uf2 をトラックボール（統括）側に、peripheral 用を反対側に書き込みます。
